  ...
  return (req, res, next) => {
    if (schema.params) {
      ...
      if (result.errors.length) {
        ...
        if (!schema.safeParse) logger.warn("Your version of strummer does not support mutateParams")
    ...
    if (schema.query) {
      ...
      if (result.errors.length) {
        ...
        if (!schema.safeParse) logger.warn("Your version of strummer does not support mutateQuery")
